# Generated by Powerlevel10k configuration wizard on 2022-08-05 at 14:36 -03.
...
# Habilitar archivos de registro de Powerlevel10k
typeset -g POWERLEVEL9K_LOG_FILE=~/.p10k.zsh.log
typeset -g POWERLEVEL9K_LOG_ERRORS_FILE=~/.p10k.zsh-errors.log
...
() {
  ...
  function prompt_ping() {
    local ping_output=$(ping -c 1 google.com 2>/dev/null | sed -n '2p')
    
    if [[ $ping_output =~ time=([0-9.]+) ]]; then
        
        local ping_time=$(printf "%.2f" "${match[1]}" | sed 's/,/./')
        p10k segment -f 3 -i '󰇖 google.com' -t "${ping_time}ms"

    else
        p10k segment -f 1 -i '󰇖 google.com' -t "Error"
    fi
  }
